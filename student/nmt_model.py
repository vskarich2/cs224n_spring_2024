...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.h_projection = nn.Linear(2 * h_size, h_size, bias=False)
        self.c_projection = nn.Linear(2 * h_size, h_size, bias=False)

        self.att_projection = nn.Linear(2 * h_size, h_size, bias=False)
        self.combined_output_projection = nn.Linear(3 * h_size, h_size, bias=False)
        self.target_vocab_projection = nn.Linear(h_size, len(vocab.tgt), bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # Here we are reducing the dimensions of our hidden state vector h.
